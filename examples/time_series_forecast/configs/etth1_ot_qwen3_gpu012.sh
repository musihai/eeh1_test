...
# RL 初始化 checkpoint；当前默认指向最新推荐的 heuristic-route SFT
export RL_MODEL_PATH="${RL_MODEL_PATH:-$PROJECT_DIR/artifacts/checkpoints/sft/time_series_forecast_sft_paper_strict_heuristicroute_4gpu_20260323/global_step_37/huggingface}"
...
# vLLM 显存利用率
export RL_ROLLOUT_GPU_MEMORY_UTILIZATION="${RL_ROLLOUT_GPU_MEMORY_UTILIZATION:-0.15}"
# 模型加载格式
...
# PPO 每卡最大 token 长度
export RL_ACTOR_MAX_TOKEN_LEN_PER_GPU="${RL_ACTOR_MAX_TOKEN_LEN_PER_GPU:-6144}"
# rollout 最大模型长度
export RL_ROLLOUT_MAX_MODEL_LEN="${RL_ROLLOUT_MAX_MODEL_LEN:-8192}"
# vLLM 每批最大 token 数
export RL_ROLLOUT_MAX_BATCHED_TOKENS="${RL_ROLLOUT_MAX_BATCHED_TOKENS:-4096}"
# vLLM 最大并发序列数
export RL_ROLLOUT_MAX_NUM_SEQS="${RL_ROLLOUT_MAX_NUM_SEQS:-4}"
# 是否强制 eager
...
# =========================
# SFT 默认使用的 GPU 列表
export SFT_GPU_IDS="${SFT_GPU_IDS:-0,1,2,3}"
# SFT 实际可见 GPU；需要临时改卡时优先覆盖这个变量
export SFT_CUDA_VISIBLE_DEVICES="${SFT_CUDA_VISIBLE_DEVICES:-$SFT_GPU_IDS}"
# 当前推荐的 SFT 数据目录
export SFT_DATASET_DIR="${SFT_DATASET_DIR:-$PROJECT_DIR/dataset/ett_sft_etth1_runtime_ot_teacher200_paper_same4_stepwise_heuristicroute}"
# SFT 保存目录
export SFT_SAVE_DIR="${SFT_SAVE_DIR:-$PROJECT_DIR/artifacts/checkpoints/sft/time_series_forecast_sft_paper_strict_heuristicroute_4gpu_20260323}"
# SFT 项目名
export SFT_PROJECT_NAME="${SFT_PROJECT_NAME:-TimeSeriesForecast-SFT-Formal}"
# SFT 使用的 GPU 数
export SFT_NUM_GPUS="${SFT_NUM_GPUS:-4}"
# SFT 最大样本长度
...
case "$RUN_MODE" in
    val_only)
        ...
        # RL val-only 默认值
        # =========================
        export RL_EXP_NAME="${RL_EXP_NAME:-etth1_ot_qwen3_1_7b_rl_paper_strict_formal_valonly_20260323}"
    ...
    smoke)
        ...
        # RL smoke 默认值
        # =========================
        export RL_EXP_NAME="${RL_EXP_NAME:-etth1_ot_qwen3_1_7b_rl_paper_strict_formal_smoke_20260323}"
    ...
    train)
        ...
        # SFT train 默认值
        # =========================
        export SFT_EXPERIMENT_NAME="${SFT_EXPERIMENT_NAME:-qwen3-1.7b-etth1-ot-sft-paper-strict-heuristicroute-4gpu-20260323}"
        ...
        # RL train 默认值
        # =========================
        export RL_EXP_NAME="${RL_EXP_NAME:-etth1_ot_qwen3_1_7b_rl_paper_strict_formal_20260323}"
        export RL_TRAIN_BATCH_SIZE="${RL_TRAIN_BATCH_SIZE:-3}"
        export RL_PPO_MINI_BATCH_SIZE="${RL_PPO_MINI_BATCH_SIZE:-3}"
        export RL_PPO_MICRO_BATCH_SIZE="${RL_PPO_MICRO_BATCH_SIZE:-1}"
        export RL_LOGPROB_MICRO_BATCH_SIZE="${RL_LOGPROB_MICRO_BATCH_SIZE:-1}"
