...
def bootstrap_metric(
    ...
    return [(np.nanmean(lst), np.nanstd(lst)) for lst in bootstrap_metric_lsts]
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                numeric_vals = [_coerce_numeric(v) for v in var_vals]
                if np.isnan(np.asarray(numeric_vals, dtype=float)).all():
                    continue

                metric = {}
                n_resps = len(numeric_vals)
                metric[f"mean@{n_resps}"] = np.nanmean(numeric_vals)

                if n_resps > 1:
                    metric[f"std@{n_resps}"] = np.nanstd(numeric_vals)
                    ...
                    for n in ns:
                        [(bon_mean, bon_std), (won_mean, won_std)] = bootstrap_metric(
                            data=numeric_vals, subset_size=n, reduce_fns=[_safe_nanmax, _safe_nanmin], seed=seed
                        )
                        ...
                        if var2vals.get("pred", None) is not None:
                            vote_data = [
                                {"val": val, "pred": pred}
                                for val, pred in zip(numeric_vals, var2vals["pred"], strict=True)
                                if not np.isnan(val)
                            ]
                            if vote_data:
                                [(maj_n_mean, maj_n_std)] = bootstrap_metric(
                                    data=vote_data,
                                    subset_size=min(n, len(vote_data)),
    ...
    for data_source, var2metric2uid_vals in data_src2var2metric2uid_vals.items():
        for var_name, metric2uid_vals in var2metric2uid_vals.items():
            for metric_name, uid_vals in metric2uid_vals.items():
                uid_numeric_vals = [_coerce_numeric(v) for v in uid_vals]
                if np.isnan(np.asarray(uid_numeric_vals, dtype=float)).all():
                    data_src2var2metric2val[data_source][var_name][metric_name] = float("nan")
                else:
                    data_src2var2metric2val[data_source][var_name][metric_name] = np.nanmean(uid_numeric_vals)
