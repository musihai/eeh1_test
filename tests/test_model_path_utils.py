...
def test_has_loadable_transformers_weights_recognizes_sharded_safetensors(tmp_path):
    model_dir = tmp_path / "model"
    model_dir.mkdir()
...
def test_resolve_transformers_model_dir_accepts_sharded_huggingface_dir(tmp_path):
    huggingface_dir = tmp_path / "huggingface"
    huggingface_dir.mkdir()
    (huggingface_dir / "config.json").write_text("{}")
    (huggingface_dir / "model.safetensors.index.json").write_text("{}")
    (huggingface_dir / "model-00001-of-00002.safetensors").write_text("shard")

    assert resolve_transformers_model_dir(huggingface_dir) == huggingface_dir
